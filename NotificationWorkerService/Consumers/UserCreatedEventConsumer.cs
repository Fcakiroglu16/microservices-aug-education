...
namespace NotificationWorkerService.Consumers;
       ...
       await _channel.QueueBindAsync(queueName, "one.api-user.created-exchange", string.Empty, null, cancellationToken: stoppingToken);
        ...
        consumer.ReceivedAsync += async (sender, eventArgs) =>
        {
            try
            {
                ...
                {
                    Console.WriteLine($"(Notification)UserCreatedEvent received: {userCreatedEvent.UserId},{userCreatedEvent.UserName}, {userCreatedEvent.Email}");
                }
